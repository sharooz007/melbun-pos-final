-- Migration: 0005_dashboard_metrics_rpc.sql (Hardened V2)

-- 1. Performance Indexes for Live Reporting Aggregations
CREATE INDEX IF NOT EXISTS idx_invoices_created_at_voided ON invoices(created_at) WHERE is_voided = FALSE;
CREATE INDEX IF NOT EXISTS idx_payments_created_at ON payments(created_at);
CREATE INDEX IF NOT EXISTS idx_expenses_created_at_voided ON expenses(created_at) WHERE is_voided = FALSE;
CREATE INDEX IF NOT EXISTS idx_variants_stock_quantity ON variants(stock_quantity);

-- 2. Hardened Dashboard Metrics RPC
...
BEGIN
    ...
    -- 9. RBAC Masking: Hide Profit from Staff
