-- Migration 0058: Full Invoice Edit RPC & Backdated Checkout Support
-- Implements Total Editability (Full CRUD) with Authoritative Financial Verification & Return Guards

-- 1. Drop old 10-parameter process_checkout to prevent PostgREST RPC candidate overload collisions
DROP FUNCTION IF EXISTS process_checkout(UUID, DECIMAL, DECIMAL, DECIMAL, BOOLEAN, DECIMAL, DECIMAL, DECIMAL, JSONB, JSONB);

-- 2. Upgrade process_checkout to accept optional p_created_at
...
BEGIN
    ...
    -- Block Uncollected Balance for Anonymous Walk-in Customers (Policy 10)
    ...
    LOOP
        ...
        VALUES (v_item_id, v_item.variant_id, 'SALE', -v_total_pieces, 'Sale: ' || v_invoice_number, v_invoice_created_at);
    END LOOP;
            ...
            IF v_payment.amount > 0 THEN
                IF v_payment.method = 'STORE_CREDIT' THEN
                    ...
                    INSERT INTO customer_credit_ledger (
                        ...
                        v_invoice_id,
                        'Store Credit checkout payment for ' || v_invoice_number,
...
-- 3. CREATE FUNCTION update_full_invoice FOR TOTAL EDITABILITY
...
BEGIN
    ...
    -- 2. Foreign Key & Return Integrity Guard (P0)
    ...
    -- Delete old SALE stock movements
    ...
    -- Authoritative Tax & Final Total Verification (P0)
    IF p_gst_applied THEN
        v_expected_cgst := ROUND(GREATEST(0, v_calculated_subtotal - p_discount_amount) * 0.025, 2);
        v_expected_sgst := ROUND(GREATEST(0, v_calculated_subtotal - p_discount_amount) * 0.025, 2);
    END IF;
    ...
    IF v_new_store_credit > 0 THEN
        ...
        IF v_customer_credit < v_new_store_credit THEN
            RAISE EXCEPTION 'Insufficient store credit balance. Available: %, Requested: %', v_customer_credit, v_new_store_credit;
        END IF;
