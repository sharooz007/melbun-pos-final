-- Migration: 0072_reports_bank_payment_breakdown.sql
-- Adds BANK payments to get_comprehensive_reports while preserving all CTEs, cutoff logic, and schema alignment
...
BEGIN
    ...
    -- 3. Collections Breakdown (Including BANK and excluding voided/hidden invoices)
    SELECT 
    ...
    -- Tab 1: Invoices List with Lateral Payment Summary
    SELECT COALESCE(jsonb_agg(
        jsonb_build_object(
            'id', sub.id,
