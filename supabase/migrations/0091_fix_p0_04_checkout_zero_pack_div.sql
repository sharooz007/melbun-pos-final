-- Migration: 0091_fix_p0_04_checkout_zero_pack_div.sql
-- Description: Fix PostgreSQL Division-by-Zero Fatal Crash in process_checkout on Zero Pack Size (P0-04)
-- Resolves:
-- 1. Division-by-zero fatal crash (22012) when variant or product pieces_per_set is 0 or negative
-- 2. Clamps effective_pps with GREATEST(1, COALESCE(v.pieces_per_set, p.pieces_per_set, 1)) in both validation and deduction passes
-- 3. Fixes integer division truncation: casts remaining stock to numeric before division: (stock_quantity - v_total_pieces)::numeric / v_variant.effective_pps
-- 4. Fixes NULL stock_sets inflation in LEAST(): uses COALESCE(stock_sets, 0) - COALESCE(v_item.sets_quantity, 0)
-- 5. Fixes validation pass bypass for NULL stock_sets: COALESCE(v_variant.stock_sets, 0) < COALESCE(v_item.sets_quantity, 0)
-- 6. Preserves strict lock ordering (variants ordered by id FOR UPDATE -> customer FOR UPDATE) from 0089
-- 7. Drops historical overloads cleanly to avoid PostgREST PGRST203 candidate ambiguity
...
-- Unqualified drops for robust search_path parity (matching 0085/0089 migration standards)
...
-- ============================================================================
-- STEP 2: RECREATE AUTHORITATIVE 13-PARAMETER process_checkout WITH ZERO-PACK PROTECTION
-- ============================================================================
