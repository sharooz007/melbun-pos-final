-- Migration: 0067_harden_checkout_bounds.sql
-- Enforces strict DB-level bounds on discount, round-off, future dates, deterministic variant locking, price anti-tampering, and overpayment across BOTH process_checkout and update_full_invoice.

-- 1. Hardened process_checkout
...
BEGIN
    ...
    -- Financial Bounds Checks (Issue 7)
    ...
    -- Future Date Buffer Guard (Buffer Policy 13)
    ...
    -- Overpayment Guard (Issue 7 & Policy 6)
    ...
    -- Walk-in Zero Credit Invariant (Policy 10)
    ...
    LOOP
        ...
        VALUES (v_item_id, v_item.variant_id, 'SALE'::stock_movement_type, -v_total_pieces, 'Sale: ' || v_invoice_number, v_invoice_created_at);
    END LOOP;
    ...
    IF p_payments IS NOT NULL AND jsonb_array_length(p_payments) > 0 THEN
        ...
        LOOP
            IF v_payment.amount > 0 THEN
                ...
                VALUES (v_invoice_id, p_customer_id, v_payment.amount, v_payment.method, v_invoice_created_at);
            END IF;
        END LOOP;
    END IF;

    RETURN jsonb_build_object('success', true, 'invoice_id', v_invoice_id, 'invoice_number', v_invoice_number, 'final_total', p_final_total);
END;
$$;

-- 2. Hardened update_full_invoice with identical financial bounds and price anti-tampering
...
BEGIN
    ...
    IF v_new_store_credit > 0 THEN
        ...
        INSERT INTO customer_credit_ledger (
            ...
            v_new_credit_balance,
            p_invoice_id,
            'Applied store credit to updated Invoice ' || v_invoice.invoice_number,
            v_invoice_created_at
        );
    END IF;

    -- 10. UPDATE INVOICE ROW
    UPDATE invoices
    ...
    -- 11. REPLACE INVOICE ITEMS & DEDUCT NEW STOCK
...
GRANT EXECUTE ON FUNCTION process_checkout(UUID, DECIMAL, DECIMAL, DECIMAL, BOOLEAN, DECIMAL, DECIMAL, DECIMAL, JSONB, JSONB, TIMESTAMPTZ) TO authenticated, service_role;
