-- Migration: 0035_harden_returns_rpc.sql
-- Fixes P0 Return Proration Exploit, Dual Inventory tracking, and Rounding Drift in returns.

-- 1. Ensure RLS is disabled on returns to match pos-v2 standard (data mutation via RPC only)
ALTER TABLE returns DISABLE ROW LEVEL SECURITY;

-- 2. Drop old 5-parameter overload from Migration 0007 to prevent routing to vulnerable RPC
DROP FUNCTION IF EXISTS process_return(UUID, INTEGER, payment_method, TEXT, TEXT);

-- 3. Create Hardened 6-Parameter Return RPC
...
BEGIN
    ...
    -- 7. Record Negative Payment Outflow
