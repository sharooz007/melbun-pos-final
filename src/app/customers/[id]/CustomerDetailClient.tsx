...
export default function CustomerDetailClient({ id }: { id: string }) {
  ...
  const handleSendWhatsAppDueReminder = (inv?: any) => {
    ...
    const message = formatWhatsAppMessage(template, {
      ...
      date: dateStr,
      item_count: inv?.invoice_items?.length || 1,
      total_amount: totalAmt,
      paid_amount: paidAmt,
      due_amount: dueAmt,
      status: `Payment Due: ₹${dueAmt.toFixed(2)}`
    });

    const cleanPhone = cleanWhatsAppPhone(customer?.phone);
    if (cleanPhone) {
    ...
    <div className="p-3.5 pb-36 sm:p-6 sm:pb-36 md:p-8 md:pb-8 max-w-6xl w-full mx-auto space-y-6">
      ...
      <div className="bg-surface border border-border rounded-2xl p-4 sm:p-6 shadow-xs space-y-5">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-2.5 sm:gap-3.5 pt-2 border-t border-border">
          ...
          <div className="bg-row-alt/60 p-3 sm:p-4 rounded-xl border border-border flex flex-col justify-between">
            <div>
            ...
            {Number(customer.pending_dues || 0) > 0 && (
              <button
                onClick={() => handleSendWhatsAppDueReminder()}
                className="text-[11px] font-bold text-[#075E54] hover:underline mt-1.5 text-left flex items-center gap-1 cursor-pointer"
              >
                          ...
                          {inv.status !== 'Paid' && inv.status !== 'Void' && inv.status !== 'Refunded' && due > 0 && (
                            <>
                              <button
                                ...
                                className="px-2.5 py-1.5 bg-[#25D366]/10 hover:bg-[#25D366]/20 text-[#075E54] border border-[#25D366]/30 rounded-xl text-xs font-bold transition min-h-[34px] flex items-center gap-1 cursor-pointer"
                                title="Send WhatsApp Due Reminder"
                              >
